...
class LatexRenderBox extends RenderBox {
	...
	@override
	double computeMinIntrinsicWidth(double height) => _computeIntrinsicSize().width;

	@override
	double computeMaxIntrinsicWidth(double height) => _computeIntrinsicSize().width;

	@override
	double computeMinIntrinsicHeight(double width) => _computeIntrinsicSize().height;

	@override
	double computeMaxIntrinsicHeight(double width) => _computeIntrinsicSize().height;

	@override
  double computeDistanceToActualBaseline(TextBaseline baseline) => entryNode.baselineOffset + baselineOffset;

	@override
	Size computeDryLayout(BoxConstraints constraints) => _computeIntrinsicSize(constraints);
	...
	void paint(PaintingContext context, Offset offset) {
		if (backgroundColor != null) {
			context.canvas.drawRect(
				...
				Paint()..style = PaintingStyle.fill..color = backgroundColor!,
			);
		}
	...
	Size _computeIntrinsicSize([BoxConstraints? constraints]) {
		...
		if (constraints != null && entryNode.size.width > constraints.maxWidth) {
			switch (wrapMode) {
			  ...
			  case LatexWrapMode.simple:
					entryNode = LineBreaksHandler.addLineBreaks(entryNode, renderContext, constraints.maxWidth);
					_customTextAlign = null;
					entryNode.performLayout(renderContext);
					break;

			  case LatexWrapMode.smart:
					var r = LineBreaksHandler.addSmartLineBreaks(entryNode, renderContext, constraints.maxWidth);
					entryNode = r.$1;
		...
		if (constraints != null) {
			return Size(
				...
				entryNode.size.height.clamp(constraints.minHeight, constraints.maxHeight),
			);
		} else {
			return entryNode.size;
		}
