...
class LineBreaksHandler {
  ...
  static EntryNode addLineBreaks(EntryNode entryNode, RenderContext renderContext, double maxWidth) {
    ...
    while	(i < lines.length) {
      ...
      i++;
      if (node is !GroupNode || node.children.length <= 1) continue; // TODO bracket terms are currently considered as single term, that cannot be split.
  ...
  static (EntryNode, MultiLineTextAlign?) addSmartLineBreaks(EntryNode entryNode, RenderContext renderContext, double maxWidth) {
    ...
    for (var node in entryNode.lines) {
      if (node is GroupNode && node.children.length > 1 && node.size.width > maxWidth) {
    ...
    while	(i < lines.length) {
      ...
      for (var j = 1; j < node.children.length; j++) {
        ...
        if (totalWidth > maxWidth) {
          ...
          if (breakpoint == 1 && i > 0) continue; // ignore if there is only 1 node, because this is most likely =, +, - from previous line break

          // split text and create new line
          List<LatexRenderNode> nextLineChildren = node.children.sublistSafe(breakpoint);
  ...
  static List<GroupNode> _splitAtArrows(GroupNode groupNode) {
    ...
    while (i < groupNode.children.length) {
      ...
      if (node is BracketNode) {
        switch (node.bracketType) {
          ...
          case BracketType.curly: openCurlies += node.bracketOrientation.isLeft ? 1 : -1; break;
          case BracketType.square: openSquares += node.bracketOrientation.isLeft ? 1 : -1; break;
          case BracketType.doubleSquare: openDoubleSquares += node.bracketOrientation.isLeft ? 1 : -1; break;
          case BracketType.round: openRounds += node.bracketOrientation.isLeft ? 1 : -1; break;
        }
        continue;
      }
      if (i == 0 || openCurlies != 0 || openSquares != 0 || openDoubleSquares != 0 || openRounds != 0) continue;
      ...
      if (node is TextNode && arrowLineBreaks.contains(node.text)) {
        ...
        LatexRenderNode? nextNode = groupNode.children.length > i ? groupNode.children[i] : null;
        if (nextNode is !TextNode || !nextNode.text.startsWith('\u2001') && !nextNode.text.startsWith('\u00a0')) {
          node.text += '\u2001';
        }
    ...
    for (var i = 1; i < breakpoints.length; i++) {
      newGroups.add(GroupNode(groupNode.children.sublistSafe(breakpoints[i-1], breakpoints[i]), BracketType.none));
    }

    return newGroups;
  }

  static int _findPreferredLineBreak(List<LatexRenderNode> nodes, int ogBreakpoint) {
    ...
    for (var i = 0; i < ogBreakpoint; i++) {
      ...
      if (node is SupSubNode && node.parent is BracketNode) node = node.parent!; // )², )_i
    ...
    double equationSplitLength = nodes.sublistSafe(0, equationBreakpoint).sumDouble((e) => e.size.width);
    double arithmeticSplitLength = nodes.sublistSafe(0, arithmeticBreakpoint).sumDouble((e) => e.size.width);
    double bracketSplitLength = nodes.sublistSafe(0, bracketBreakpoint).sumDouble((e) => e.size.width);

    // bracket
    if (bracketBreakpoint > 0 && bracketSplitLength*0.5 > equationSplitLength && bracketSplitLength*0.7 > equationSplitLength) {
      return bracketBreakpoint+1;
    }

    // arithmetic
    if (arithmeticBreakpoint > 0 && arithmeticSplitLength*0.7 > equationSplitLength) {
