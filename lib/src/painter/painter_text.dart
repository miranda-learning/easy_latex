...
class LatexTextPainter extends TextPainter {
  static double fontSizeToHeight(double fontSize) =>
      (fontSize * 1.173).roundToDouble();
  ...
  factory LatexTextPainter.withRenderContext(
      String text, RenderContext renderContext) {
    return LatexTextPainter(
      text,
  ...
  static TextSpan _buildTextSpan(String text, double fontSize, LatexFont? font,
      bool isBold, Color color, Color? bgColor) {
    return TextSpan(
      text: text,
      style: TextStyle(
  ...
  Size get size {
    ...
    double width =
        wR < w && (w - wR) < w * 0.00042 ? w.floorToDouble() : w.ceilToDouble();
