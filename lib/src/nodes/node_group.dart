...
class GroupNode extends LatexRenderNode {
	...
	void performLayout(RenderContext renderContext, {isDense = false}) {
		...
		RenderContext context = color != null ? renderContext.copyWith(color: color) : renderContext;
		...
		for (var i = 0; i < children.length; i++) {
			...
			groupBaselineToBottom = max(groupBaselineToBottom, children[i].size.height - children[i].baselineOffset);

			// spacing
			if (i == children.length-1 || (children[i] is NumberNode && children[i+1] is TopDecorationNode)) { // second part required for 1.\dot9 (1.9 periodic)
				spacings.add(0);
			} else {
		...
		if (bracketType != BracketType.none) {
			_leftBracket = BracketPainter(context, groupHeight, bracketType, BracketOrientation.left);
			_rightBracket = BracketPainter(context, groupHeight, bracketType, BracketOrientation.right);
		...
		if (children.isNotEmpty) {
			spacingRequired = children.first.spacingRequired || children.last.spacingRequired;
		}
	}
	...
	void paint(Canvas canvas, double start, double baseline) {
		...
		double startOffset = _leftBracket != null ? (_leftBracket!.size.width + _bracketSpacing) : 0;
		for (var i = 0; i < children.length; i++) {
	...
	void computeCursorPositions(double start, double baseline, List<Offset> offsets, List<double> fontSizes) {
		...
		// group
		for (var i = 0; i < children.length; i++) {
			children[i].computeCursorPositions(start + startOffset, baseline, offsets, fontSizes);
			startOffset += children[i].size.width + spacings[i];
		}

		// right bracket
		if (_rightBracket != null && children.isNotEmpty && children.last is !PlaceholderNode) {
			offsets.add(Offset(start + size.width - _rightBracket!.size.width - fontSize*0.08, baseline)); fontSizes.add(fontSize);
		}
	}
	...
	@override
	double get topCenterOffset => children.length == 1 ? children.first.topCenterOffset : 0;
	...
	@override
	double get topRightOffset => children.isNotEmpty ? children.last.topRightOffset : 0;
