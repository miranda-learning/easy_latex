...
class MatrixNode extends LatexRenderNode {
	...
	MatrixNode(List<List<LatexRenderNode>> rows, {this.bracketType = BracketType.round}) {
		_rows = rows;
	}

	MatrixNode.fromGroup(GroupNode group, {this.bracketType = BracketType.round}) {
		...
		for (var child in children) {
			if (child is NewColumnNode) {
				if (rows.last.last.children.isEmpty) rows.last.last.children.add(TextNode(''));
	...
	void performLayout(RenderContext renderContext, {isDense = false}) {
		...
		// brackets
		_leftBracket = BracketPainter(renderContext, height, bracketType, BracketOrientation.left);
		_rightBracket = BracketPainter(renderContext, height, bracketType, BracketOrientation.right);
	...
	void paint(Canvas canvas, double start, double baseline) {
		...
		for (var i = 0; i < _rows.length; i++) {
			...
			for (var j = 0; j < _rows[i].length; j++) {
				...
				_rows[i][j].paint(canvas,
					start + columnOffset + _columnWidths[j]/2 - _rows[i][j].size.width/2,
		...
		_rightBracket.paint(canvas, start + size.width - _rightBracket.size.width, baseline);
	}
