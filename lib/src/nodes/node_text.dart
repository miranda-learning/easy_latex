...
class TextNode extends LatexRenderNode {
	...
	void performLayout(RenderContext renderContext, {isDense = false}) {
		...
		_textPainter = LatexTextPainter(text,
			fontSize: fontSize,
	...
	void computeCursorPositions(double start, double baseline, List<Offset> offsets, List<double> fontSizes) {
		...
		for (var i = 0; i < text.length; i++) {
			...
			offsets.add(Offset(start + _textPainter.getOffsetForCaret(TextPosition(offset: i), Rect.zero).dx, baseline));
	...
	double get topCenterOffset {
		if (_font is MainItalicLatexFont) {
			switch (textType) {
				case TextType.upperCase: return fontSize * 0.11;
				case TextType.mediumCase: return fontSize * 0.05;
				case TextType.lowerCase: return fontSize * 0.05;
			}
		}
	...
	@override
	double get topRightOffset => _font is MainItalicLatexFont ? calcTopRightOffset(fontSize) : 0;
	...
	bool _hasDescenders(String text) {
		return (text.contains('f') && _font is MainItalicLatexFont || text.contains('g') || text.contains('j') || text.contains('p') || text.contains('q') || text.contains('y'));
	}
