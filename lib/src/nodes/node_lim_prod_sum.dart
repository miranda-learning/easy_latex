...
class LimProdSumNode extends LatexRenderNode {
	...
	void performLayout(RenderContext renderContext, {isDense = false}) {
		...
		// layout parent
		_parent.performLayout(renderContext.copyWith(fontSizeScaling: _fontScaling));


		// layout super
		RenderContext subSupContext = renderContext.copyWith(fontSize: (renderContext.fontSize*0.7).roundToDouble());
		...
		if (superscript != null) {
			...
			if (superscript!.size.height > renderContext.fontSize) {
				superscript!.performLayout(
					subSupContext.copyWith(fontSize: (1.2 * subSupContext.fontSize * renderContext.fontSize/superscript!.size.height).roundToDouble()),
					isDense: true
				);
			}
		...
		if (subscript != null) {
			...
			if (subscript!.size.height > renderContext.fontSize) {
				subscript!.performLayout(
					subSupContext.copyWith(fontSize: (1.2 * subSupContext.fontSize * renderContext.fontSize/subscript!.size.height).roundToDouble()),
					isDense: true
				);
			}
		...
		size = Size(
			...
			_parent.baselineOffset + superBaselineToTop + max(subHeight, _parent.size.height - _parent.baselineOffset),
		);
	...
	void paint(Canvas canvas, double start, double baseline) {
		_parent.paint(canvas,
			start + size.width/2 - _parent.size.width/2,
			baseline,
		);

		if (superscript != null) {
			superscript!.paint(canvas,
