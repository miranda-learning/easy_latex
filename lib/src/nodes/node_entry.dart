...
class EntryNode extends LatexRenderNode {
  ...
  void performLayout(RenderContext renderContext, {isDense = false}) {
    ...
    if (lines.twoOrMore) {
      height +=
          (lines.length - 1) * renderContext.mainFontSize * lineSpacingFactor;
    }
    ...
    if (height == 0) {
      LatexTextPainter painter =
          LatexTextPainter.withRenderContext('', renderContext)..layout();
      size = painter.size;
  ...
  void paint(Canvas canvas, double start, double baseline,
      {double? width, MultiLineTextAlign? textAlign}) {
    if (lines.length == 1) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      baseline += line.size.height -
          line.baselineOffset +
          (renderContext?.mainFontSize ?? 0) * lineSpacingFactor;
    }
  }
  ...
  void computeCursorPositions(double start, double baseline,
    ...
    offsets.add(Offset(start + size.width, baseline));
  ...
  String toStringWithIndent(String indent) {
    String str =
        '$indent$runtimeType: w: ${size.width.toStringAsFixed(2)}, h: ${size.height.toStringAsFixed(2)}, baseline: ${baselineOffset.toStringAsFixed(2)}';
