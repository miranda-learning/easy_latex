...
class FracNode extends LatexRenderNode {
	...
	void performLayout(RenderContext renderContext, {isDense = false}) {
		...
		double denominatorHeight = _isDense ? (denominator.size.height - denominator.baselineOffset + denominator.baselineToTop) : denominator.size.height;
		double numeratorHeight = _isDense ? (numerator.size.height * 0.86 + _lineWidth) : numerator.size.height;

		size = Size(
			max(numerator.size.width, denominator.size.width) + _lineWidth + horizontalMultilineOffset,
	...
	void paint(Canvas canvas, double start, double baseline) {
		numerator.paint(canvas,
			start + size.width/2 - numerator.size.width/2,
			baseline - baselineOffset - _lineWidth*0.5 + numerator.baselineOffset
		);

		denominator.paint(canvas,
			start + size.width/2 - denominator.size.width/2,
			baseline - _lineOffset + _lineWidth*0.5 + (_isDense ? (denominator.baselineToTop + _lineWidth) : denominator.baselineOffset)
		);
	...
	void computeCursorPositions(double start, double baseline, List<Offset> offsets, List<double> fontSizes) {
		offsets.add(Offset(start - fontSize*0.1, baseline)); fontSizes.add(fontSize);
		...
		if (!(numerator is PlaceholderNode || numerator is GroupNode && (numerator as GroupNode).children.isNotEmpty && (numerator as GroupNode).children.last is PlaceholderNode)) {
			offsets.add(Offset(
				start + size.width/2 + numerator.size.width/2,
				baseline - baselineOffset - _lineWidth*0.5 + numerator.baselineOffset
			));
		...
		if (!(denominator is PlaceholderNode || denominator is GroupNode && (denominator as GroupNode).children.isNotEmpty && (denominator as GroupNode).children.last is PlaceholderNode)) {
			offsets.add(Offset(
				start + size.width/2 + denominator.size.width/2,
				baseline - _lineOffset + _lineWidth*0.5 + (_isDense ? (denominator.baselineToTop + _lineWidth) : denominator.baselineOffset),
			));
