...
class PlaceholderNode extends LatexRenderNode {
	...
	void performLayout(RenderContext renderContext, {isDense = false}) {
		...
		baselineOffset = LatexTextPainter.fontSizeToBaseline(renderContext.fontSize);
	}
	...
	void paint(Canvas canvas, double start, double baseline) {
		...
		if (type != PlaceholderType.dashed) {
			...
			Paint paint = Paint()
				..color = type == PlaceholderType.lightRed || type == PlaceholderType.red ? const Color(0xFFFFCDD2)
					: renderContext!.color.withAlpha(16)
				..style = PaintingStyle.fill;

			canvas.drawRect(rect, paint);
			if (type == PlaceholderType.red) {
				canvas.drawRect(rect, Paint()..color = const Color(0xFFF44336)..style = PaintingStyle.stroke);
			}

		// dashed placeholder
		} else {

			double fontSizeRatio = renderContext!.fontSize / renderContext!.mainFontSize;
			double strokeWidth;
			...
			canvas.drawDashedLine(Offset(l_x - strokeWidth/2, t_y), Offset(r_x - strokeWidth/2, t_y), linePaint, [strokeWidth, strokeWidth]);
			canvas.drawDashedLine(Offset(r_x, t_y - strokeWidth/2), Offset(r_x, b_y - strokeWidth/2), linePaint, [strokeWidth, strokeWidth]);
			canvas.drawDashedLine(Offset(r_x + strokeWidth/2, b_y), Offset(l_x + strokeWidth/2, b_y), linePaint, [strokeWidth, strokeWidth]);
			canvas.drawDashedLine(Offset(l_x, b_y + strokeWidth/2), Offset(l_x, t_y + strokeWidth/2), linePaint, [strokeWidth, strokeWidth]);
		}
	}
