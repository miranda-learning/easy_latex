...
class NumberNode extends LatexRenderNode {
	...
	void performLayout(RenderContext renderContext, {isDense = false}) {
		...
		_formattedText = NumberFormatter.formattedNumber(text, locale: renderContext.locale) ?? 'Error formatting number';
		_textPainter = LatexTextPainter.withRenderContext(_formattedText, renderContext);
		_textPainter.layout();
	...
	@override
	double get baselineToTop => (renderContext?.font?.isSansSerif ?? false) ? fontSize * 0.8 : fontSize * 0.72;
	...
	@override
	String toStringWithIndent(String indent) => '$indent$runtimeType: text: $text, formatted: $_formattedText, '
