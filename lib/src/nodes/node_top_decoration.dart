...
enum TopDecoration {
	...
	String? get decorationChar {
		switch (this) {
			...
			case dddot: 							return '\u02d9\u02d9\u02d9';
			case hat: 								return '\u005e';
			case overbrace: 					return null;
			case overline: 						return null;
			case overleftarrow:  			return null;
			case overleftrightarrow: 	return null;
			case overrightarrow: 			return null;
			case tilde: 							return '\u02dc';
			case vec: 								return '\u20d7';
			case widehat: 						return null;
			case widetilde: 					return null;
		}
	}
...
class TopDecorationNode extends LatexRenderNode {
	...
	void paint(Canvas canvas, double start, double baseline) {
		double offset = 0;
		if (child.size.width < size.width) offset += (size.width - child.size.width)/2;
	...
	@override
	double get baselineToTop => child.baselineToTop + (decoration == TopDecoration.overbrace ? fontSize*0.4 : fontSize*0.24);


	void _drawTextDecoration(Canvas canvas, double start, double baseline) {
		double x = start + size.width / 2 - _textPainter.size.width / 2 + child.topCenterOffset;
	...
	void _drawPathDecoration(Canvas canvas, double start, double baseline) {
		if (decoration == TopDecoration.overline ||
			...
			if (decoration == TopDecoration.overleftarrow || decoration == TopDecoration.overleftrightarrow) {
				double t = x - fontSize * 0.04;
				canvas.drawLine(Offset(t + arrowOffset, y - arrowOffset), Offset(t, y), paint);
				canvas.drawLine(Offset(t + arrowOffset, y + arrowOffset), Offset(t, y), paint);
			}

			if (decoration == TopDecoration.overrightarrow || decoration == TopDecoration.overleftrightarrow) {
