...
class LatexSpan extends WidgetSpan {
	...
	LatexSpan({
		...
		String? locale,
			...
			super(
			child: _buildChild(text, style, backgroundColor, wrapMode, parsing, allPointsAsDecimalPoints, locale),
	...
	static Widget _buildChild(
		...
		try {
			return LatexRenderWidget(
				...
				renderContext: RenderContext(fontSize: style?.fontSize, color: style?.color, locale: locale),
			...
			return Text(
				text,
				style: TextStyle(fontSize: style?.fontSize, color: Colors.yellow, backgroundColor: Colors.red),
			);
		}
	}
